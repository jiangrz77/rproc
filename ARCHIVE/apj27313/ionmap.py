...
* orgions seems not yet implemented
* addions seems not yet implemented
* keepions seems not adequately implemented, just like stabions.
* the difference between keepions and stabions seems only affect EL/A output?

* a new strategy is needed

isomer -> isotope -> Element
                     Isobar
                     Isotone
...
from starfit.autils.human import byte2human
from starfit.autils.human import time2human
from starfit.autils.human import version2human

from starfit.autils.isotope import ion as I
from starfit.autils.isotope import Ion

from starfit.autils.isotope import VOID, PROTON, NEUTRON, ALPHA, isomermap
from starfit.autils.isotope import ufunc_A, ufunc_Z, ufunc_N
from starfit.autils.isotope import ufunc_idx, ufunc_isotope_idx
from starfit.autils.isotope import ufunc_is_isomer, ufunc_is_isobar, ufunc_is_isotone, \
    ufunc_is_element, ufunc_is_isotope, ufunc_isotope_idx, \
    ufunc_ion_from_idx


from .reaction import DecIon
from starfit.autils.abuset import IonList, AbuData, IonSet, AbuSet
from starfit.autils.abusets import SolAbu
from starfit.autils.utils import contract, project, iterable
from starfit.autils.utils import CachedAttribute, cachedmethod
from starfit.autils.logged import Logged
...
class IonMap(Logged):
    ...
    def input2ionarr(ions, molfrac_in = None, molfrac_out = None):
        ...
        if not isinstance(ions[0], Ion):
            ions = np.array([I(ix) for ix in ions])
    ...
    def __call__(self, data,
        ...
        s = data.shape
        result = np.dot(data.reshape(-1, s[-1]), self.map).reshape(*s[:-1], -1)
...
class Isomer2Isotope(IonMap):
    def __init__(
        ...
        a = np.identity(len(ions))
        ii = ufunc_is_isomer(ions)
        if not np.any(ii):
            self.map = a
            self.decions = ions
            return
        idx = ufunc_isotope_idx(ions)
        self.map, idx = project(a, idx, return_values=True, axis=1)
        self.decions = ufunc_ion_from_idx(idx)

        self.close_logger()

def isomer2isotope(*args, **kwargs):
    return Isomer2Isotope.Map(*args, **kwargs)
...
class Decay(IonMap):
    ...
    def __init__(self,
                 ...
                 solabu = None,
                 keepall = True,
                 decayfile = '/home/jiangrz/ssd/GitHub/EMPs_IMF/rproc/apj27313/decay.dat',
                 isomers = None,
                 silent = False,
                 debug = False,
                 ):
        ...
        TODO - apply to isomer/istope mappting?
        ...
        decayfile - decay file to use.
        TODO - allow to pass an object

        isomers - process as isomers (gs?) if input is isotopes

        silent - whether to be verbose or not
        """
        self.setup_logger(silent)

#        ions = np.array([I('pb220')])
#        ions = np.array([I('n13'), I('o14'), I('pb209')])
#        ions = np.array([I('n13'), I('o14')])
        ...
        if ionlist is not None:
            ionlist = I(ionlist)
            ionlist = np.atleast_1d(np.squeeze(ionlist))
        ...
        if addions is not None:
            addions = I(addions)
            addions = np.atleast_1d(np.squeeze(addions))
        ...
        if need_elements or need_isobars or need_isotones:
            need_stable = decay
            print(f' [DECAY] Warning - setting stable to {need_stable}')
            # TODO - this result may be unexpected if elements in ionlist
            #        tiggers stable
        ...
        if stabions is not None:
            ...
            # TODO - Q: what happens if we provide isotope and isomer?
        ...
        # we loose info about decayed data we may need for A/EL sums?
        if keepions is not None:
            # the strategy is to modify the decdata
            for ix in keepions:
                self.decdata.add_stable(ix)
            # TODO - later, these contibutions need to be removed for sums
        ...
        # TODO - test overlap in keepions and stabions and addions

        if need_elements or need_isobars or need_isotones:
            if keepions is not None:
                ii = np.in1d(raw_ions, keepions)
                assert np.count_nonzero(ii) == len(keepions)
                ii = ~ii
                proj_ions = raw_ions[ii]
                proj_matrix = raw_matrix[:,ii]
            else:
                proj_matrix = raw_matrix
                proj_ions = raw_ions
        if need_elements:
            decionz = ufunc_Z(proj_ions)
            elements_decmatrix, z = project(proj_matrix, decionz, return_values = True, axis = -1)
            elements_decions = np.array([I(Z = Z) for Z in z])
        if need_isobars:
            deciona = ufunc_A(proj_ions)
            isobars_decmatrix, a = project(proj_matrix, deciona, return_values = True, axis = -1)
            isobars_decions = np.array([I(A = A) for A in a])
        if need_isotones:
            decionn = ufunc_N(proj_ions)
            isotones_decmatrix, n = project(proj_matrix, decionn, return_values = True, axis = -1)
            isotones_decions = np.array([I(N = N) for N in n])

        # *** likely, this should not use raw_ions but self.decions
        if need_isotopes:
            deciiso = ufunc_isotope_idx(raw_ions)
            isotopes_decmatrix, xions = project(raw_matrix, deciiso, return_values = True, axis = -1)
            isotopes_decions = np.array([I(idx = ion) for ion in xions])
        ...
        if ionlist is not None:
            ...
            decmatrix = []
            for i in ionlist:
                # TODO - Add other cases, optimise
                if i.is_isotope:
                    ionx = self.decions
                    matx = self.decmatrix
                    # ionx = isotopes_decions
                    # matx = isotopes_decmatrix
                elif i.is_element:
                    ionx = elements_decions
                    matx = elements_decmatrix
                elif i.is_isobar:
                    ionx = isobars_decions
                    matx = isobars_decmatrix
                elif i.is_isotone:
                    ionx = isotones_decions
                    matx = isotones_decmatrix
                else:
                    raise AttributeError(f'ionlist - not supported: {i}')

                jj = np.where(ionx == i)[0]
                if len(jj) == 0:
                    decmatrix.append(np.zeros(len(self.ions)))
                else:
                    j = jj[0]
                    decmatrix.append(matx[:,j])
        ...
        # not sure this really should be done
        if keepions is not None:
            ii = ~np.in1d(keepions, decions)
            jj = ~np.in1d(keepions, addions)
            kk = ii & jj
            if np.any(kk):
                if addions is None:
                    addions = keepions[kk]
                else:
                    np.hstack((keepions[kk], addions))

        if addions is not None:
            # TODO
            # raise NotImplementedError(f'addions {addions}')
            addmatrix = []
            for i in addions:
                # TODO - deal with isomers
                if i.is_isotope:
                    ionx = self.decions
                    matx = self.decmatrix
                elif i.is_element:
                ...
                jj = np.where(ionx == i)[0]
                if len(jj) == 0:
                    addmatrix.append(np.zeros(len(self.ions)))
                else:
                    j = jj[0]
                    addmatrix.append(matx[:,j])
            addmatrix = np.array(addmatrix).T
            decmatrix = np.hstack((decmatrix, addmatrix))
            decions = np.hstack((decions, addions))
...
class DecayData(Logged):
    ...
    def __init__(self,
        ...
        with open(self.decayfile,'rt') as f:
            ...
            for s in f:
                ...
                assert ix != VOID, "Ion name not valid: " + str(ix)
                i = 1
                if len(reac) > i:
                    try:
    ...
    def get_decay(self, ix):
        ...
        try:
            ...
            Z = ix.Z
            a = np.array([jx.A for jx in self.decdata.keys() if jx.Z == Z and jx.E == 0])
            assert len(a) > 0, f"Problem finding isotopes for decay {ix!s}."
            assert not (min(a) < ix.A < max(a)), 'decay chain has gaps, no unique extrapolation possible'
            A = min(max(a), max(min(a), ix.A))
            refion = I(Z = Z, A = A, isomer = ix.is_isomer)
            try:
    ...
    def add_stable(self, ix):
        ...
        if len(a) > 0:
            ...
            if self.isomers:
                ...
                E = None
            if A <= amin:
                sel = list(range(A + 1, amin)) + [A - 1]
            elif A >= amax:
                sel = list(range(amax + 1, A)) + [A + 1]
            else:
                sel = list()
            for a in sel:
                newion = I(Z = Z, A = a, E = E)
...
_factorial = [1, 1, 2, 6, 24, 120, 720, 5760, 51840, 518400]
...
class TimedDecay(IonMap):
    def __init__(self,
        ...
        print(f' [{self.__class__.__name__}] using {molfrac_in=}, {molfrac_out=}')
        ...
        for d in decays:
            ...
            r = min(d.rate, lim * 2**binpow)
            # if len(ii) > 1:
            #     # for ions that have multiplicity in 'in' channel:
            #     # need to devide by n! (not relevant for decays)
            #     c = Counter(ii)
            #     r /= np.prod([_factorial(v) for v in c.values()])
            # else:
            #     c[ii[0]] = 1
            assert len(ii) == 1
    ...
    def update_time(self, time = None, lim=None, silent=None):
        ...
        with self.timeenv(timing = 'decay matrix computed in'):
            a = self.a0 * (time * 0.5**self.binpow)
            if lim is not None:
                a = np.maximum(-lim, np.minimum(lim, a))
            else:
                if np.any(a > 1000):
                    self.logger.warn(
                        'decay matrix may cause overflow, ' +
                        'consider using lim=1e-6*dt for __init__')
            a = expm(a)
            for _ in range(self.binpow):
                a = a @ a
            # a = powm(a, 2**self.binpow)
            self._project(a)
        self.time = time
        self.close_logger()
    ...
    def _project(self, a):
        ...
        a = a[ii, :][:,io]
        self.map = self.molfrac_convert(a)
    ...
    def __call__(self, *args, **kwargs):
        ...
        if time is None and len(args) > 1:
            ...
            args = args[:1] + args[2:]
        lim = kw.pop('lim', None)
        if lim is None and len(args) > 1:
            lim = args[1]
            args = args[:1] + args[2:]
        if time is not None:
            self.update_time(time, lim)
    ...
    def decay_data(
        ...
        if amed:
            ...
            for k,v in ranges.items():
                ranges_[k] = sorted(set(v))
            ranges = ranges_

            aimax = np.max([i.A for i in ions])
            zimax = np.max([i.Z for i in ions])
            zdlist = list(ranges.keys())
            zdmax = np.max(zdlist)
            zmax = np.maximum(zimax, zdmax)
            decaydata = copy(decaydata)

            rlim = np.exp(20.)
            add_count = 0
            for Z in range(2, 83):
                for A in range(int(Z * 1.5), ranges[Z][0]):
                    decaydata.append(DecayRate([I(Z=Z, A=A)], [I(Z=Z-1, A=A)], rlim))
                    add_count += 1
                for A in range(ranges[Z][-1], int(Z*3 + 8 + 0.01*Z**2)):
                    decaydata.append(DecayRate([I(Z=Z, A=A)], [I(Z=Z+1, A=A)], rlim))
                    add_count += 1
            for Z in range(83, zmax + 1):
                for A in range(int(Z * 1.5), np.minimum(int(Z*3 + 8 + 0.01*Z**2), aimax) + 1):
                    if Z in zdlist and A in ranges[Z]:
                        continue
                    if A < 2.5 * Z:
                        decaydata.append(DecayRate([I(Z=Z, A=A)], [I(Z=Z-1, A=A)], rlim))
                    elif Z < 92:
                        ...
                        decaydata.append(DecayRate([I(Z=Z, A=A)], [I(Z=Z-2, A=A-4), I.He4], rlim))
                    add_count += 1

            self.logger_info(f'Amended {add_count} artifical decays')
        ...
        # add all internal decays
        for d in decaydata:
            if d.ions <= ions:
                if not d in selected:
                    selected.append(d)
                    self.logger.warn(
                        "adding missed decay " + str(d))
